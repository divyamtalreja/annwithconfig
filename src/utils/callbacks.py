...
def get_timestamp(name):
    timestamp = time.asctime().replace(" ", "_").replace(":", "_") #asctime  it will get given informat sat oct 23 time year 
    unique_name = f"{name}_at_{timestamp}"
    return unique_name

def get_callbacks(config, X_train):
    logs = config["logs"]
    unique_dir_name = get_timestamp("tb_logs")#name of folder is tb_logs
